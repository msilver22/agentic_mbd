...
st.set_page_config(
    page_title="Icebreaker App for Farcaster",
)

st.title("Icebreaker App")
...
def json_filler(state: IcebreakerState):
    ...
    json_prompt = (
        ...
        - Create macro categories for broad topics and micro categories for specific interests.\n
...
if "agent" not in st.session_state:
    st.session_state.icebreaker = builder.compile()
...
if "state" not in st.session_state:
    st.session_state.state = IcebreakerState(
...
# Truncate the conversation at same point 
if st.session_state.state["conversation_count"] == 2:
    st.write("Here are the keywords collected during the conversation:")
    st.json(st.session_state.state["all_keywords"])
    st.info("The chat has ended. Thank you for participating!")
    st.stop()

# Handle user input
if prompt := st.chat_input("Write anything"):
    ...
    if len(st.session_state["messages"]) == 1:
        response = st.session_state.icebreaker.invoke({"messages": messages, "summary": None, "all_keywords": {}}, st.session_state.config)
    else:
        response = st.session_state.icebreaker.invoke({"messages": messages, "summary": st.session_state.state["summary"], "all_keywords": {}, "conversation_count": st.session_state.state["conversation_count"]}, st.session_state.config)


    # Display the assistant's response
    bot_reply = response["messages"][-1].content
    st.session_state.state["conversation_count"] += 1
    st.session_state.state["all_keywords"] = response["all_keywords"]
    st.session_state.state["summary"] = response["summary"]
